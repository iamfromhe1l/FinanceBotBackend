...
export class DebtsToMeController {
    ...
    @Post('add')
    async addDebtsToMe(@Body() dto: AddDebtsToMeDto): Promise<DebtsToMeModel|UserModel> {
        return this.debtsToMeService.addDebtsToMe(dto);
    }

    @Delete()
    async deleteDebtsToMe(@Body() dto: RemoveDebtsToMeDto): Promise<DebtsToMeModel|number> {
    ...
    @Put()
    async editDebtsToMe(@Body() dto: EditDebtsToMeDto): Promise<DebtsToMeModel|number> {
